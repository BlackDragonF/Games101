...
type Triangle struct {
	...
	v [3]common.Vec3f
	/* Per vertex values */
	color      [3]common.Vec3f // color at each vertex;
...
func NewTriangle() Triangle {
	return Triangle{
		...
		color:      [3]common.Vec3f{{0., 0., 0.}, {0., 0., 0.}, {0., 0., 0.}},
...
func (triangle *Triangle) SetColor(ind int, r float64, g float64, b float64) error {
	if ind < 0 || ind > 2 {
		return errors.New(fmt.Sprintf("triangle: ind %d out of range\n", ind))
	}
	if r < 0. || r > 255. || g < 0. || g > 255. || b < 0. || b > 255 {
		return errors.New(fmt.Sprintf("triangle: invalid color value (%.2f, %.2f, %.2f)\n", r, g, b))
	}
